...
app.post('/consent', (request, response) => {
	reqChecker += 1;
	if (reqChecker % 3 === 0) {
		response.redirect('http://localhost:8000/surprise/html/intro.html?type=1');
	} else if (reqChecker % 3 === 1) {
		response.redirect('http://localhost:8000/surprise/html/intro.html?type=2');
	} else {
		response.redirect('http://localhost:8000/vsup/html/intro.html');
	}
})
