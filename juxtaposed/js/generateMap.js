...
function makeMaps(){
	...
	if (expType != 0) {
		...
		if (rnd_gen == 0){
			...
			document.getElementById('txt-b').textContent = "The Choropleth Map shows sales data, weighted directly by population as of 10/12/2022."
		} else {
			document.getElementById('txt-a').textContent =  " The Choropleth Map shows sales data, weighted directly by population as of 10/12/2022."
			document.getElementById('txt-b').textContent = "The Surprise Map summarizes counties with interesting sales rates based on the national average. A county can show either high surprise, low surprise or no surprise."
		}
	}
