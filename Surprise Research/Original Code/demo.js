...
function makeMaps(){
  ...
  //console.log(Math.min(...sData), Math.max(...sData), document.getElementById("yearLabel").innerHTML)
  update();
}
...
function makeBigMap(theScale, theData, theTitle, id){
  ...
  .enter()
  .append("path")
  .datum(function(d){ d[id] = theData[d.properties.NAME]; 
					  sData.push(d[id][parseInt(document.getElementById("yearLabel").innerHTML) - 1981]);
					  return d;})
  .attr("d",path)
  .attr("fill","#333")
  .append("svg:title")
...
function update(){
  ...
  sData.push(d.surprise[curYear - minYear]);
